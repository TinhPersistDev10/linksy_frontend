...
interface Options {
  ...
  replaceOptimistic: (tempId: string, confirmed: Message) => void;
  removeOptimistic:  (tempId: string) => void;
  signalRSend:       (chatroomId: string, text: string, type: string) => Promise<void>;
...
export function useSendMessage({
  ...
  signalRSend, signalRTyping, signalRStopTyping,
}: Options) {
  ...
  const notifyTyping = useCallback(async (value: string) => {
  ...
  const handleSend = useCallback(async () => {
    ...
    appendOptimistic({
      ...
      replyCount: 0,
      isEdited: false, isDeleted: false, isOwn: true,
      sentAt: new Date().toISOString(),
      editedAt: null, deletedAt: null, attachments: null,
    });
    ...
    try {
      ...
      try {
        const sent = await chatroomsApi.sendMessage(chatroomId, { messageText: content, messageType: "text" });
  ...
  }, [input, chatroomId, sending, user, stopTypingNow, appendOptimistic, signalRSend, replaceOptimistic, removeOptimistic]);
