...
import {
  Send, MoreVertical, Phone, Video, Smile,
  Paperclip, ArrowLeft, Trash2
} from 'lucide-react';
...
function useChatSignalRClient(options: {
  ...
  useEffect(() => {
    ...
    const manage = async () => {
      if (currentRoomRef.current && currentRoomRef.current !== options.chatroomId) {
  ...
  const sendMessage = useCallback(async (chatroomId: string, text: string, type = 'text') => {
    const conn = connectionRef.current;
    if (!conn) throw new Error('Not connected');
    await conn.invoke('SendMessage', { ChatroomId: chatroomId, MessageText: text, MessageType: type });
  }, []);
  ...
  const deleteMessage = useCallback(async (chatroomId: string, messageId: string) => {
    await connectionRef.current?.invoke('DeleteMessage', chatroomId, messageId);
  }, []);

  return { isConnected: isMounted && isConnected, sendMessage, sendTyping, stopTyping, deleteMessage };
}
...
export default function ChatWindowLayout({ chatroom, onBack }: ChatWindowLayoutProps) {
  ...
  const handleReceiveMessage = useCallback((message: Message) => {
    setMessages(prev => {
      ...
      const withoutTemps = message.isOwn
        ...
        : prev;
      if (withoutTemps.some(m => m.messageId === message.messageId)) return withoutTemps;
  ...
  const handleUserTyping = useCallback(({ userId, username, chatroomId: roomId }: any) => {
  ...
  const { isConnected, sendMessage, sendTyping, stopTyping, deleteMessage } = useChatSignalRClient({
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      try {
        ...
        setMessages(prev => prev.map(m => m.messageId === tempId ? sent : m));
      } catch {
  ...
  return (
    <div className="flex-1 flex flex-col h-full overflow-hidden">
      ...
      <div className="flex items-center gap-3 px-4 py-3 border-b bg-background/80 backdrop-blur-sm shrink-0">
        ...
        <Avatar src={otherMember?.avatar} name={otherMember?.fullname || chatroom.roomName} />
        <div className="flex-1 min-w-0">
          <p className="font-semibold text-sm truncate">{otherMember?.fullname || chatroom.roomName}</p>
      ...
      <div className="flex-1 overflow-y-auto px-4 py-4 space-y-1">
        {loading ? (
          ...
          <div className="flex flex-col items-center justify-center h-full gap-2 text-center">
            <Avatar src={otherMember?.avatar} name={otherMember?.fullname || ''} size={14} />
          ...
          <>
                ...
                <div key={msg.messageId}>
                  ...
                  <div className={cn(
                    ...
                    <div className="w-7 h-7 shrink-0">
                      {showAvatar && (
                        <Avatar src={msg.senderAvatar} name={msg.senderFullname} size={7} />
                      )}
                    </div>
            ...
            {typingUsers.length > 0 && (
              <div className="flex items-end gap-2 mt-3">
                <div className="w-7 h-7 shrink-0">
                  <Avatar src={otherMember?.avatar} name={typingUsers[0].username} size={7} />
                </div>
        ...
        <div className="flex items-end gap-2 bg-muted/50 rounded-2xl border px-3 py-2">
          ...
          <button
            ...
            className={cn(
              ...
              input.trim() ? 'bg-blue-500 text-white hover:bg-blue-600' : 'text-muted-foreground'
            )}>
            <Send size={16} />
          </button>
        </div>
